...
func getTestCacheProgramPath(t *testing.T) string {
	return path.Join(getTestCachePath(t), pkg.ProgramName)
}

func TestWriteCacheAndReadCacheEqualsExpected(t *testing.T) {
	gitmojis := pkg.Gitmojis{}

	err := json.Unmarshal([]byte(testData), &gitmojis)
	assert.NoError(t, err)
	getCacheDirPersist := utils.GetCacheDir

	utils.GetCacheDir = func(programName string) (string, error) {
		testCachePath := getTestCachePath(t)
		return path.Join(testCachePath, programName), nil
	}
	pkg.CacheGitmojis(gitmojis)
	gitmojisFromCache, err := pkg.GetGitmojisCache()
	defer func() {
		utils.GetCacheDir = getCacheDirPersist
		os.RemoveAll(getTestCacheProgramPath(t))
	}()
	assert.NoError(t, err)
	assert.Equal(t, gitmojis, gitmojisFromCache)
}

func TestFindGitmoji(t *testing.T) {
	caseArgs := []string{":rocket:", "🚀"}
	...
	var testFkt = func(emojiOrCode string) func(t *testing.T) {
		return func(t *testing.T) {
			found := pkg.FindGitmoji(emojiOrCode, gitmojis.Gitmojis)
			assert.NotNil(t, found)
			exp := pkg.Gitmoji{Emoji: "🚀", Entity: "&#x1f680;", Code: ":rocket:", Desc: "Deploy stuff.", Name: "rocket", Semver: ""}
			assert.Equal(t, exp, *found)
		}
	}

	for _, arg := range caseArgs {
